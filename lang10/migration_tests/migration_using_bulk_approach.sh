...
auction_to_load=$(echo $EXPECTED_AUCTION | jq .data.result | jq -r tostring)

# Recreating our auction from v1.0.0 in v2.0.0
curl --request POST --header "Symbiont-Key-Alias: $key_alias" \
--url http://localhost:8888/api/v1/contracts/auction/10-2.0.0/import_auctions \
--header 'Content-Type: application/json' --silent --data "{\"auctions\":[[$auction_to_load,\"PUBLIC\"]]}"

echo ">>> IMPORTED $auction_to_load"

sleep 2

# Retrieve our auction in v2.0.0
IMPORTED_AUCTION=$(curl --request POST --header "Symbiont-Key-Alias: $key_alias" \
--url http://localhost:8888/api/v1/contracts/auction/10-2.0.0/get_auction_by_id \
--header 'Content-Type: application/json' --silent --data "{\"id\":\"$auction_id\"}")

echo ">>> IMPORTED RETRIEVED $IMPORTED_AUCTION"

sym local-network stop
[[ $(echo "$IMPORTED_AUCTION" | jq .data.result.id) == $(echo "$EXPECTED_AUCTION" | jq .data.result.id) ]] && \
[[ $(echo "$IMPORTED_AUCTION" | jq .data.result.creator) == $(echo "$EXPECTED_AUCTION" | jq .data.result.creator) ]] && \
[[ $(echo "$IMPORTED_AUCTION" | jq .data.result.last_bid) == $(echo "$EXPECTED_AUCTION" | jq .data.result.last_bid) ]] && \
[[ $(echo "$IMPORTED_AUCTION" | jq .data.result.product_description) == $(echo "$EXPECTED_AUCTION" | jq .data.result.product_description) ]] && \
echo "Successful migration" || echo "FAILURE"
