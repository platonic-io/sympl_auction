...
def do_an_auction(auction, key_alias, bidder, second_bidder, other_key_alias, third_key_alias, network):
    ...
    for idx in range(1, max_n * 2):
      bidder = other_key_alias if idx < max_n else third_key_alias
      async_call(network, bidder, AUCTION, "bid", {"id": new_auction['id'], "amount" :price_str.format(price=100+idx)})
    wait_till(lambda  : not(str(auction.last_bid_for_auction(id=new_auction["id"])) == price_str.format(price=100 + (max_n*2) -1)))
