...
AUCTION = ContractRef('auction_ric', '1.0.0', 10)
...
class TestScaleAuction():
    ...
    def auction(self, network, reset_publish, key_alias):
        return network[key_alias].auction_ric['10-1.0.0']

    @pytest.fixture
    def bidder(self, network, reset_publish, other_key_alias):
        return network[other_key_alias].auction_ric['10-1.0.0']

    @pytest.fixture
    def second_bidder(self, network, reset_publish, third_key_alias):
        return network[third_key_alias].auction_ric['10-1.0.0']
